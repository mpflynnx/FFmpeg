...
void checkasm_check_example(void)
{
    ...
    void (*add_bytes_ptr)(uint8_t *, uint8_t *, int) = ff_add_bytes_c;

    // 3. Declare the function signature. 
    // Format: (return_type, arg1, arg2, arg3...)
    declare_func(void, uint8_t *dst, uint8_t *src, int len);
    ...
    if (check_func(ff_add_bytes_c, "add_bytes")) {
    ...
    add_bytes_ptr = ff_add_bytes_sse2; // Make sure this is declared/linked!

    if (check_func(add_bytes_ptr, "add_bytes_sse2")) {
        declare_func(void, uint8_t *dst, uint8_t *src, int len);
        
        call_ref(dst_ref, src, BUF_SIZE); // Runs C version
        call_new(dst_new, src, BUF_SIZE); // Runs SSE2 version
